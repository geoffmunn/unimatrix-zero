...
def create(max_number, line_length, picked, cover, testmode, path):
    ...
    DB_FILE_NAME = 'coverage.db'

    delete_combos_table: str = "DROP TABLE IF EXISTS combos;"
    create_combos_table = "CREATE TABLE combos (ID INTEGER PRIMARY KEY AUTOINCREMENT, line TEXT NOT NULL);"
    ...
    insert_combo: str  = "INSERT INTO combos (line) VALUES (?);"
    ...
    conn.execute(delete_combos_table)
    conn.commit()
    
    conn.execute(create_combos_table)
    #conn.execute(create_index)
    conn.commit()

    cursor = conn.cursor()

    # Generate the line length combos and save them to the database
    ...
    set_combos = combinations(range_set, line_length)
    print (set_combos)
    for line in list(set_combos):
        print ('line:', line)
        conn.execute(insert_combo, [' '.join(str(v) for v in line)])

        # Now generate the subsets

        subset_combos = combinations(line, 5)
        for subset_line in list(subset_combos):
            print (subset_line)
