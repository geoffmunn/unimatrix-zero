...
def create(max_number, line_length, picked, cover, testmode, path):
    ...
    for i in range(1, lines_from_picked + 1):
        ...
        if i > bottom_index and i not in covered_picked_csns:
            ...
            max_coverage_count1	= 0
            max_candidate_line1	= []
            max_current_csns1	= set()

            best_coverage_count = 0
            best_candidate_line1 = []
            best_candidate_line2 = []
            best_current_csns1 = set()
            best_current_csns2 = set()
            ...
            for subset in covered_subsets_template:
                ...
                for missing_length_subset in missing_length_template:
                    ...
                    single_line_coverage_total = coverage_total + len(current_csns)

                    if single_line_coverage_total < lines_from_picked:

                        cur_line2 = cur_line[:]
                        cur_line2 = zero_functions.next_combination(cur_line2, max_number)

                        covered_picked_csns1 = covered_picked_csns.copy()

                        for csn in current_csns:
                            if csn > bottom_index:
                                covered_picked_csns1.add(csn)

                        # Step 1: Take the next line of $picked length
                        for i2 in range(i, lines_from_picked + 1 - 1):

                            ###############################
                            # Do not process this line if it's already covered
                            if i2 > bottom_index and i2 not in covered_picked_csns1:

                                # Reset the maximum statistics because we're starting a new line search
                                max_coverage_count2	= 0
                                max_candidate_line2	= []
                                max_current_csns2	= set()

                                covered_picked_csns2 = covered_picked_csns1.copy()

                                # Step 2: Go through each subset of $cur_line. These are of size $covered from $picked
                                # $covered cannot be larger than $line_length (or $picked)
                                loop_count2	= 0
                                loop_max2	= len(covered_subsets_template)
                                for subset2 in covered_subsets_template:
                                    loop_count2 += 1

                                    # Map each number in $cur_line to a spot in the subset template
                                    templated_subset2 = []
                                    for j2 in subset2:
                                        templated_subset2.append(cur_line2[j2])

                                    # Find the missing numbers from this subset
                                    missing_subset_numbers2 = []
                                    for j2 in range(1, max_number + 1):
                                        if j2 not in templated_subset2:
                                            missing_subset_numbers2.append(j2)
                                    ...
                                    # Step 3: Now combine them to lines of length $line_length - these are potential nominated lines
                                    #quick_exit = False
                                    for missing_length_subset2 in missing_length_template:

                                        # This full line is a candidate for what can cover the current line
                                        candidate_line2 = templated_subset2[:]
                                        for j2 in missing_length_subset2:
                                            candidate_line2.append(missing_subset_numbers2[j2])
                                        candidate_line2.sort()
                                        ...
                                        # Step 4: Go through each covered subset (length = line_length) in the candidate line
                                        for covered_subset2 in covered_subsets_length_template:

                                            # Map each spot in the subset to a $candidate_line number
                                            templated_covered_subset2 = []
                                            for j2 in covered_subset2:
                                                templated_covered_subset2.append(candidate_line2[j2])
                                            templated_covered_subset2.sort()

                                            # Find the missing numbers from this subset
                                            missing_candidate_subset_numbers2 = []
                                            for j2 in range(1, max_number + 1):
                                                if j2 not in templated_covered_subset2:
                                                    missing_candidate_subset_numbers2.append(j2)

                                            # Step 5: Now build this up to $picked
                                            for missing_picked_cover_subset2 in missing_picked_cover_template:

                                                # Add the missing numbers onto the templated line:
                                                covered_picked_line2 = templated_covered_subset2[:]
                                                for j2 in missing_picked_cover_subset2:
                                                    covered_picked_line2.append(missing_candidate_subset_numbers2[j2])
                                                covered_picked_line2.sort()

                                                # This full line is a candidate for what can cover $cur_line
                                                # Get the CSN so we can check if we've already got it:
                                                csn2 = zero_functions.sequence_number(covered_picked_line2, max_number)

                                                # If the CSN is below the bottom-most index, then we can assume it's covered
                                                # This should be a bit quicker for very large wheels
                                                if csn2 > bottom_index:
                                                    if csn2 not in covered_picked_csns2:
                                                        current_csns2.add(csn2)
                                                else:
                                                    below_count2.add(csn2)

                                        # The coverage is the total number of CSNs that aren't in the $covered_picked_csns list
                                        differences2 	= current_csns2.difference(covered_picked_csns2)
                                        coverage_count2 	= len(differences2)

                                        if coverage_count + coverage_count2 > best_coverage_count:
                                            best_coverage_count = coverage_count + coverage_count2
                                            best_candidate_line1	= candidate_line
                                            best_candidate_line2	= candidate_line2

                                            best_current_csns1	= current_csns.copy()
                                            best_current_csns2	= current_csns2.copy()

                                        #exit()

                                print ('---')
                                print ('best line1:', best_candidate_line1)
                                print ('best line2:', best_candidate_line2)
                                print ('two line coverage:',best_coverage_count)
                            ################################
                            cur_line2 = zero_functions.next_combination(cur_line2, max_number)
            ...
            final_lines.append(best_candidate_line1)
            if len(best_candidate_line2) != 0:
                final_lines.append(best_candidate_line2)

            # Update the list of covered CSNs
            for csn in best_current_csns1:
                if csn > bottom_index:
                    covered_picked_csns.add(csn)

            for csn in best_current_csns2:
                if csn > bottom_index:
                    covered_picked_csns.add(csn)

            # This is the total number of covered CSNs so far.
            line1_coverage_total = coverage_total + len(best_current_csns1)
            line2_coverage_total = coverage_total + len(best_current_csns1) + len(best_current_csns2)

            coverage_total += len(best_current_csns1) + len(best_current_csns2)
            ...
            if testmode == False:
                ...
                f.write(str(len(final_lines) - 1) + ' (' + str('{:.2f}'.format((line1_coverage_total/lines_from_picked) * 100)) + '%): '  + ' '.join([str(item) for item in best_candidate_line1]) + "\n")
                if len(best_candidate_line2) != 0:
                    f.write(str(len(final_lines)) + ' (' + str('{:.2f}'.format((line2_coverage_total/lines_from_picked) * 100)) + '%): '  + ' '.join([str(item) for item in best_candidate_line2]) + "\n")
                f.close()
    ...
    if testmode == False:
        ...
        print ("Total number of lines:", len(final_lines))
